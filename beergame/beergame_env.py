...
def calc_round(game_pk):
    ...
    if game.turns_played == 4:
        previous_round = [0, 12, 0, 0, 12, 0,
                          0, 12, 0, 0, 12, 0, 0, 0, 0, 0, 0]
    else:
    ...
    backlog_retailer = state[-4]
    backlog_wholesaler = state[-3]
    backlog_distributor = state[-2]
    backlog_manufacturer = state[-1]
